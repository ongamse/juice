  ...
  return async ({ body, connection }: Request, res: Response, next: NextFunction) => {
    ...
    try {
      if (email === users.admin.email) {
        ...
        const validToken = adminUser != null && token != null && await isValidResetPasswordToken(adminUser, token)

        if (adminUser && validToken && token) {
...
function verifyResetTokenChallenges (user: UserModel, token: string) {
  challengeUtils.solveIf(challenges.resetPasswordAdminChallenge, () => {
    return user.id === users.admin.id && token === createResetPasswordToken(user.email)
  })
}
